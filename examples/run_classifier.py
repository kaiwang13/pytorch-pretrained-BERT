    ...
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch", disable=args.local_rank not in [-1, 0]):
            ...
            if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                # model = BertForSequenceClassification.from_pretrained(epoch_output_dir, num_labels=num_labels)
                # tokenizer = BertTokenizer.from_pretrained(epoch_output_dir, do_lower_case=args.do_lower_case)

                # Good practice: save your training arguments together with the trained model
                output_args_file = os.path.join(epoch_output_dir, 'training_args.bin')
